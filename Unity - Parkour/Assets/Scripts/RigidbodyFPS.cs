...
namespace UnityStandardAssets.Characters.FirstPerson
{
	...
	public class RigidbodyFPS : MonoBehaviour
	{
		...
		{
			public float ForwardSpeed = 8.0f;   // velocidade pra frente
			public float BackwardSpeed = 4.0f;  // velocidade pra trás
			public float StrafeSpeed = 4.0f;    // velocidade para os lados 
			public float SpeedInAir = 8.0f;   // velocidade no ar
		...
		{
			//evita que o mouse apareça quando o jogo for pausado
			if (Mathf.Abs(Time.timeScale) < float.Epsilon) return;

			//pega a rotação antes de alterar
